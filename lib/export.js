...
module.exports = function(ttyu_js_c) {
    ...
    var ttyutil = createObjectWithSealedMethods({
    ...
    function createObjectWithSealedMethods(methods) {
        var obj = {};
        var props = {};
    ...
    ttyutil.MouseEvent = function(type, button, x, y, ctrl) {
        if(!(utils.startsWith(type, "MOUSE") && type in ttyutil.EVENT))
            throw new TypeError("type is no valid mouse event type");
        return {
            type: type,
            button: button,
            x: x,
    ...
    // handle exits to clean up terminal
    // TODO(@bbuecherl): only handle exits when no other listener is registered
    /*process.on("exit", ttyutil.stop);
    process.on("uncaughtException", ttyutil.stop);
    Object.keys(ttyutil.SIGNAL).forEach(function(sig) {
        process.on(sig, ttyutil.stop);
    });*/
