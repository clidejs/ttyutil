...
#define UTIL_RENDER_FGBG "\x1b[38;5;%dm\x1b[48;5;%dm%s\x1b[49m\x1b[39m"
#define UTIL_RENDER_FG "\x1b[38;5;%dm%s\x1b[39m"
#define UTIL_RENDER_BG "\x1b[48;5;%dm%s\x1b[49m"

const char *util_render(const char *ch, int fg, int bg) {
    char *out = (char *)malloc(sizeof(char) * (strlen(ch) + 30));
    if(fg != -1 && bg != -1) {
        sprintf(out, UTIL_RENDER_FGBG, fg, bg, ch);
    } else if(fg != -1) {
        sprintf(out, UTIL_RENDER_FG, fg, ch);
    } else if(bg != -1) {
        sprintf(out, UTIL_RENDER_BG, bg, ch);
    } else {
        delete out;
        return ch;
    }
