...
void TTYInputWorker::Execute(const ExecutionProgress& progress) {
    ...
    new_mode = ((old_mode | ENABLE_MOUSE_INPUT | ENABLE_EXTENDED_FLAGS |
        ...
        for(i = 0; i < readed; ++i) {
            ...
            if(MOUSE_EVENT == ir[i].EventType) {
                progress.Send(const_cast<const ttyutil_event*>(
                    ttyutil_event_create(EVENT_MOUSE, ttyutil_mouse_create(
                            ...
                            GetMouseAction(ir[i].Event.MouseEvent.dwEventFlags,
                                    ir[i].Event.MouseEvent.dwButtonState),
                            GetCtrlCodes(
                                    ir[i].Event.MouseEvent.dwControlKeyState))))
                    );
            } else if(KEY_EVENT == ir[i].EventType) {
                progress.Send(const_cast<const ttyutil_event*>(
                        ttyutil_event_create(EVENT_KEY, ttyutil_key_create(
                            GetCtrlCodes(
